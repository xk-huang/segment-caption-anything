"""
Copyied from `.modeling_sca_multitask_v2.py` commit d68cc8b and modified by Xiaoke.
1. Add timm
"""
import collections
import math
...
from .configuration_sca import ScaTimmConfig, ScaMaskCaptionDecoderConfig
...
class ScaMaskCaptionMultitaskV2Decoder(nn.Module):
    ...
    def forward(
        ...
        # NOTE: resize the CLIP feature to the same size as the prompt embeddings
        _, _, height, width = dense_prompt_embeddings.shape
        image_embeddings = F.interpolate(image_embeddings, size=(height, width))
        ...
        # NOTE: Not using ambiguous outputs
        ...
        # NOTE: fix bug. point_embeddings is the input, but point_embedding is the output
        # NOTE: input & output have different name: point_embedding**s** -> point_embedding
        point_embeddings = torch.cat([caption_tokens, point_embedding], dim=-2)
...
class ScaTimmVisionNeck(nn.Module):
...
class ScaMultitaskPretrainedModel(SamPreTrainedModel):
    config_class = ScaTimmConfig
    base_model_prefix = "sca_multitask"
    main_input_name = "pixel_values"


class ScaTimmMultitaskV2Model(ScaMultitaskPretrainedModel):
    ...
    def __init__(self, config: ScaTimmConfig, language_model: nn.Module = None):
        super().__init__(config)
        self.shared_image_embedding = SamPositionalEmbedding(config.vision_config)

        # NOTE: replace sam vision encoder with timm's.
        logger.info(f"Replace vision encoder with timm's: {config.timm_vision_name}")
        self.timm_vision_encoder = timm.create_model(config.timm_vision_name, pretrained=True)
        self.neck = ScaTimmVisionNeck(config.vision_config)
        # if self.timm_vision_encoder.embed_dim != config.vision_config.hidden_size:
        self.prompt_encoder = SamPromptEncoder(config.prompt_encoder_config, self.shared_image_embedding)
        # NOTE(xiaoke): Modified. We need to outputs one more tensor: `query_outputs` for captioning
        # Thus its real name is `mask_caption_decoder`, but we keep the name `mask_decoder` for loading SAM weights.
        self.mask_decoder = ScaMaskCaptionMultitaskV2Decoder(config.mask_caption_decoder_config)
    ...
    def get_input_embeddings(self):
        return self.timm_vision_encoder.get_input_embeddings()
    ...
    def get_image_embeddings(
        ...
        pixel_values_resized = F.interpolate(
            pixel_values, size=self.timm_vision_encoder.pretrained_cfg["input_size"][-2:]
        )
        # NOTE: the mean and std are different in original CLIP, but it seems to be OK: https://github.com/openai/CLIP/issues/20
        image_embeddings = self.timm_vision_encoder.forward_features(pixel_values_resized)
        num_prefix_tokens = self.timm_vision_encoder.num_prefix_tokens
        image_embeddings = image_embeddings[:, num_prefix_tokens:, :]

        if len(image_embeddings.shape) == 3:
            batch_size, height_width, num_channels = image_embeddings.shape
            height = width = int(math.sqrt(height_width))
            image_embeddings = image_embeddings.permute(0, 2, 1).reshape(batch_size, num_channels, height, width)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if pixel_values is not None:
            ...
            # torch.Size([1, 256, 64, 64]) original, the input pixel_values torch.Size([1, 3, 1024, 1024])
    ...
    @classmethod
    def from_sam_timm_text_pretrained(
        cls,
        ...
        **kwargs,
        ...
        config = ScaTimmConfig.from_sam_timm_text_configs(
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        ...
        self.timm_vision_encoder.gradient_checkpointing = value
