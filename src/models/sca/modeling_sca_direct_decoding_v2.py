"""
Copyied from `.modeling_sca_direct_decoding.py` commit 9879823 and modified by Xiaoke.
Copied from `.modeling_sca` commit dec0744 and modified by Xiaoke.
1. Support take token prefix.
2. Fix bug in multitask model, which do not use fused query tokens in text feature mixer
3. Support controlling chunk generation size
"""
import collections
import math
...
class ScaMaskCaptionDirectDecodingV2Decoder(nn.Module):
    def __init__(self, config: ScaMaskCaptionDecoderConfig):
        ...
        self.num_caption_tokens = 1
        self.num_caption_heads = self.num_mask_tokens
...
class ScaDirectDecodingV2Model(ScaDirectDecodingPretrainedModel):
    ...
    def __init__(self, config: ScaConfig, language_model: nn.Module = None):
        ...
        self.mask_decoder = ScaMaskCaptionDirectDecodingV2Decoder(config.mask_caption_decoder_config)
