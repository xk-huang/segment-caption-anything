...
class ScaAutomaticMaskCaptionGenerator:
    ...
    def _process_batch(
        ...
        # NOTE: the point shape of SCA is (batch_size, num_masks, num_points_per_mask, 2), and it is a list
        ...
        # NOTE: binarize=False, otherwise we cannot compute stability score
        ...
        # NOTE: masks List[Tensor(num_masks, num_heads, H, W)]
        # NOTE: iou_scores Tensor(batch_size, num_masks, num_heads)
        # NOTE; captions List[str] (batch_size)

        masks = masks[0]
        iou_scores = iou_scores[0]
        # Serialize predictions and store in MaskData
        # masks.shape: torch.Size([64, 3, 534, 800])
        # iou_preds.shape: torch.Size([64, 3])
        # points.shape: (64, 2)
        data = MaskCaptionData(
        ...
        # NOTE: self.predictor.model.mask_threshold = 0.0 by default. segment_anything/modeling/sam.py
